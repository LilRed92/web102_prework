...
document.getElementById("searchInput").addEventListener("keyup", (event) => {
  ...
  let filteredGames = GAMES_JSON.filter((games) => {
    return games.name.toLowerCase().includes(searchQuery);
  })
...
const unfundedGamesString = unfundedGames.length === 1 ? "game" : "games";

// create a new DOM element containing the template string and append it to the description container
const description = document.createElement("p");
description.innerHTML = `
        <p>Our company has raised a total of $${totalRaised.toLocaleString(
          "en-US"
        )} from ${totalContributions.toLocaleString("en-US")} backers for ${
  GAMES_JSON.length
} games. Currently, ${
  unfundedGames.length
} ${unfundedGamesString} are not yet fully funded.</p>
    `;
